...
- **Bounded contexts** (canonical for new features): each is its own top-level `src/<context-name>/` directory laid out as `application/{actions,controllers,dtos}`, `domain/{entities,interfaces,services}`, `infrastructure/{*.repository.ts,*.adapter.ts}` — see `docs/architecture/domain-driven-design.md` for the binding layout and dependency rules (`domain/` imports nothing from Prisma/NestJS transport/HTTP). No bounded context is implemented yet as of the 2026-08-30 reset; `user-management/` and `access-control/` are pending redesign.
