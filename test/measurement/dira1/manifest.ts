...
export const DIRA1_PROTOCOL = 'DIRA1-MVP-v1';
export const DIRA1_MANIFEST = 'DIRA1-MANIFEST-v1';
...
/** Canonical DIRA1-MANIFEST-v1 JSON: compact UTF-8 JSON, sorted keys and fixed decimals. */
...
export const reserveArtifact = async (
  ...
  const path = join(directory, `dira1-${role}-${runId}.json`);
  const handle = await open(path, 'wx');
  try {
    const timestamp = new Date().toISOString();
    await handle.writeFile(
      `${JSON.stringify({ run_id: runId, role, protocol_version: DIRA1_PROTOCOL, manifest_version: DIRA1_MANIFEST, status: 'INCOMPLETE', created_at: timestamp, updated_at: timestamp, stop_reason: 'pending' })}\n`,
      'utf8',
    );
  } finally {
    await handle.close();
