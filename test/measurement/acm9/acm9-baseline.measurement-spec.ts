...
const ARTIFACT_DIRECTORY = resolve(__dirname, '../../../../../_bmad-output/test-artifacts/performance');
type Gate = 'reporting' | 'direct_pp' | 'colleague' | 'mixed';
type Measurement = { value_ms: number; error: null };
type GateResult = { gate: Gate; shape: string; depth: number; target_count: number; warm_up_count: number; sample_count: number; samples: Measurement[]; p50_ms: number; p95_ms: number; worst_ms: number; query_count: number; plan_reference: string };

const errorDetail = (error: unknown) => ({ error_class: error instanceof Error ? error.constructor.name : typeof error, error_message: error instanceof Error ? error.message : String(error) });
const isTimeout = (error: unknown) => /57014|statement timeout|canceling statement/i.test(error instanceof Error ? error.message : String(error));
...
describe('ACM9 PostgreSQL baseline (explicit opt-in)', () => {
  ...
  const publishIncomplete = async (error: unknown, stopReason: string) => {
    ...
    await finalizeArtifact(artifact, { status: 'INCOMPLETE', stop_reason: stopReason, ...errorDetail(error), completed_gates: completed, first_breach: firstBreach, plans });
  };
  ...
  const capturePlan = async (gate: Gate, depth: number): Promise<string> => {
    if (!prisma) throw new Error('Prisma unavailable');
    const id = `${gate}-${depth}`;
    const plan = await prisma.$queryRawUnsafe('EXPLAIN (ANALYZE, BUFFERS, FORMAT JSON) SELECT r."userId" FROM "relationships" r WHERE r."type" = \'direct\'::"RelationshipType" LIMIT 1');
    plans.push({ id, source: 'representative direct relationship hot-path predicate; captured outside timed facade samples', plan });
    return id;
  };
  ...
  const measure = async (gate: Gate, depth: number) => {
    ...
    try {
      ...
      for (let sample = 0; sample < SAMPLES; sample += 1) samples.push(await call());
      const values = samples.map((sample) => sample.value_ms);
      const result: GateResult = { gate, shape: depth === 5 ? 'balanced-depth-5' : 'acyclic-chain', depth, target_count: TARGET_COUNT, warm_up_count: WARM_UPS, sample_count: SAMPLES, samples, p50_ms: round(nearestRank(values, 0.5)), p95_ms: round(nearestRank(values, 0.95)), worst_ms: round(Math.max(...values)), query_count: (WARM_UPS + SAMPLES) * 3, plan_reference: planReference };
      completed.push(result);
      if (result.p95_ms > LIMIT_MS || result.worst_ms > LIMIT_MS) firstBreach = { gate, shape: result.shape, depth, reason: result.p95_ms > LIMIT_MS ? 'p95_ms' : 'worst_ms', p95_ms: result.p95_ms, worst_ms: result.worst_ms };
    } catch (error) {
      if (isTimeout(error)) firstBreach = { gate, shape: depth === 5 ? 'balanced-depth-5' : 'acyclic-chain', depth, reason: 'statement_timeout', ...errorDetail(error) };
      else throw error;
    }
  };
  ...
  it('runs only the requested baseline protocol and preserves an auditable result', async () => {
    ...
    try {
      ...
      try { await clean(); } finally { if (moduleRef) await moduleRef.close(); }
    }
  });
});
