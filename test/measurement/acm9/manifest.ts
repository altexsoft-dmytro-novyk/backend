...
const numberJson = (value: number): string => {
  if (!Number.isFinite(value) || !Number.isSafeInteger(value) && Math.floor(value) === value) throw new Error('Manifest numbers must be finite safe integers or decimals');
...
export const manifestHash = (manifest: Json): string => createHash('sha256').update(Buffer.from(canonicalJson(manifest), 'utf8')).digest('hex');
export const nearestRank = (values: number[], percentile: number): number => {
  if (values.length === 0) throw new Error('Cannot calculate a percentile of no samples');
...
export const resolveStatus = ({ breach, comparable }: { breach: unknown | null; comparable: boolean }): ArtifactStatus => breach !== null ? 'FAIL' : comparable ? 'PASS' : 'INCOMPLETE';

export const reserveArtifact = async (directory: string, role: RunRole, runId: string): Promise<ReservedArtifact> => {
...
export const isCompatibleBaseline = (baseline: Record<string, unknown> | null, expected: { protocolVersion: string; fixtureHash: string; environmentHash: string }): boolean => baseline?.status === 'PASS' && baseline.protocol_version === expected.protocolVersion && baseline.fixture_manifest_hash === expected.fixtureHash && baseline.environment_manifest_hash === expected.environmentHash;
